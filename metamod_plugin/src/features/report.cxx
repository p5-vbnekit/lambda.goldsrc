...
namespace p5::lambda::metamod_plugin::features::report {
namespace private_ {
...
auto trace_target_(Client const &client) noexcept(true) {
    using Trace_ = parent_::parent_::parent_::parent_::common::trace::Result;
...
auto handle_client_command_(Client &client) noexcept(false) {
    ...
    if constexpr (true) {
        ...
        if (! pointer_) return;
        if ("lambda" != ::std::string_view{pointer_}) return;
        pointer_ = engine_.functions.cmd_Argv(1);
        if ("report" != ::std::string_view{pointer_}) return;
    }
    using PrintType_ = parent_::parent_::parent_::parent_::common::PrintType;
