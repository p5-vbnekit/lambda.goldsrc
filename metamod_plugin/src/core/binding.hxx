...
namespace related_ = parent_::parent_::parent_::parent_::parent_::game;
...
namespace related_ = parent_::parent_::parent_::parent_::parent_::engine;

struct ChangeLevel final {
    using Prototype = ::std::remove_reference_t<decltype(
        *(related_::Functions::changeLevel)
    )>;
};
...
template <class T> inline consteval static auto is_engine() noexcept(true) {
    return this_::private_::is_one_of<T,
        parent_::targets::engine::ChangeLevel
    >();
}
